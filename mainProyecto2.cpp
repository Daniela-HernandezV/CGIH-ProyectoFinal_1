...
int main()
{
	...
	pointLights[0] = PointLight(1.0f, 1.0f, 1.0f,		// color de la luz // R,G,B
		...
		-122.0f, 5.0f, -180.0f,							// Posicion // X,Y,Z
	...
	pointLights3[0] = PointLight(0.0f, 1.0f, 0.0f,		// color de la luz // R,G,B
		3.0f, 1.0f,										// ambIntensidad, diffIntensidad
		-122.0f, 0.0f, -180.0f,							// Posicion // X,Y,Z
		0.3f, 0.2f, 0.1f);								// con, lin, exp
	pointLightCount3++;
	...
	{
		cameraAvatar = Camera(glm::vec3(mainWindow.getPosCamaraX(), 6.0f, mainWindow.getPosCamaraZ()), glm::vec3(0.0f, 1.0f, 0.0f), -mainWindow.getAngPsyduck() + 90.0f, 0.0f, 0.3f, 0.5f);
		...
		//Nadder alas
		modelaux = model;
		model = glm::translate(model, glm::vec3(1.3f, abs(sin(glm::radians(movHeliy * 0.1f))), -0.5f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(1.0f, 1.0f, 1.0f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
